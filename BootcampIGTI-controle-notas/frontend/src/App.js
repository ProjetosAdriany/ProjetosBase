...
export default function App() {
  ...
  const handlePersistData = async (formData) =>{
    ...
    const newGrates = Object.assign([], allGrades);
    
    const gradeToPersist = newGrates.find((grade) => grade.id === id);
  ...
  return (
    <div>
      <h1 className="center">Constrole de Notas</h1>
      {allGrades.length === 0 && <Spinner />}
      {allGrades.length > 0 && <GradesControl grades={allGrades} onDelete={handleDelete} onPersist={handlePersist}/>}
      {isModalOpen && (<ModalGrade selectedGrade={selectedGrade} onClose={handleClose} onSave={handlePersistData} />
      )}
    </div>
  )
}
